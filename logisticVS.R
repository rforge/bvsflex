...
logisticVS <- function(X, Y, b, v, Pi, block, MCMC, thin=1, seed=1234, outdir=NULL){
...
#	or a (#subsamples x p) matrix of subsamples, if for each i (=1,...,p) an empirical distribution for pi_i is provided.
  ...
  if(ncol(Pi) != p){
    if(length(Pi) == 1) {
      Pi <- as.matrix(rep(Pi, p), nrow=1);
    }else{
      if(length(Pi) == p) {
	Pi <- as.matrix(Pi, nrow=1);
      }else{
  ...
  if(!is.wholenumber(thin) | MCMC<=thin) stop("Every 'thin'-ed MCMC iteration will be kept (i.e. if 'thin'=1 then all iterations are kept, if 'thin'=10 then only every tenth iteration is returned for posterior inference. Consequently, 'thin' has to be a positive integer.\n")
