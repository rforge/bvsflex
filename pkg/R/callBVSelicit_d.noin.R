callBVSelicit_d.noin=function(k,shrink,ylim=c(0,55)){
  library(bvsflex)
  #  dir <- paste("BVS_",dvec[i],"_",format(Sys.time(), "%Y-%m-%d--%Hh%M"), sep="")
  dir <- paste("BVS_","d.noin", "_p",p,"_k",k,"_shrink",shrink,sep="")
  #  dirvec[i,2]=dir
  print(dir)
  ...
  tryerr <- try({
    elicit<-elicitBeta(d=d.noin+shrink,modelsize=k,rangeModelsize=rangeModelsize,coverage=coverage, crit="mode",lowerBound.aBeta=1, upperBound.aBeta=10000)
    pdf("betadists.pdf")
    for(iii in 1:p) {
